...
form.addEventListener("submit", (e) => {
  ...
  if (userEmail.value === "") {
    createErrorMessage("Prašome įvesti savo el. paštą");
    return;
  }

  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(userEmail.value)) {
    // tikrina kad iki "@" nebutu tarpu ir '@' simbolio
    // po to tikrina kad po "@" nebutu tusciu tarpu ir '@' simbolio
    // ir tikrina kad po "." nebutu taip pat
    createErrorMessage("Įveskite teisingą el. pašto adresą");
    return;
}
  ...
  data.usermail = userEmail.value.trim(); //pridejau userio pasta
